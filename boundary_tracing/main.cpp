#include<iostream>
#include <vector>
#include <opencv2/opencv.hpp> // image load and visualize
  ...
  while (true) {
      ...
      do {
        ...
        for (int i = 0; i < 8; i++) {
          ...
          if (in_image(next, width, height) 
              && image[next.y][next.x] == 1
              && visisted[next.y][next.x] != 1
              && is_border(image, next.x, next.y)) {
...
void visualize(std::vector<std::vector<int>> & image, std::vector<std::vector<Point>> & contours) 
{
  ...
  for (int i = 0; i < contours.size(); i++) {
    ...
    for (int j = 0; j < contours[i].size() - 1; j++) {
      cv::line(color, cv::Point(contours[i][j].x, contours[i][j].y),
      cv::Point(contours[i][j+1].x, contours[i][j+1].y), c, 1);
    }
  }
